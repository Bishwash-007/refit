    ...
    <section className="px-6 lg:px-32 py-24">
      ...
      <div className="flex flex-col lg:flex-row items-start justify-between gap-12">
        <div className="flex flex-col gap-6 max-w-xl">
          ...
          <h3 className="leading-tight">Home Improvement Specialists</h3>
        </div>
      ...
      <div
        id="gallery"
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-24 mb-32 lg:-mx-32"
      >
        {["1", "2", "3"].map((img) => (
          <img
            key={img}
      ...
      <div id="statistics">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          {stats.map((stat) => (
            <div key={stat.title} className="flex flex-col items-start gap-3">
              <h3>{stat.value}</h3>
              <p className="text-dark font-semibold p-body-m">{stat.title}</p>
              <p className="p-body-s text-dark ">{stat.desc}</p>
            </div>
          ))}
        </div>
      </div>
